...
/* ──────────────────────────────────────────────────────────────────────────
   The Doc Mirror — Resources content engine (Markdown data layer)

   ⚠️  THIS FILE IS THE ORIGINAL lib/resources.js, MOVED HERE UNCHANGED.

   It is now the FALLBACK layer. lib/resources.js became a thin orchestrator
   that prefers the CMS and falls back to this file whenever the CMS is
   unreachable, times out, errors, or returns something unusable.

   Nothing in the logic below was modified — only this header was added — so it
   can be diffed against git history and trusted as the known-good path. The
   Markdown files in content/resources/ therefore remain a working offline copy
   of the site's content, which is a stronger safety net than a purely
   CMS-backed site has.
